...
It seems that on average most steps is made at `r timemax` and it's `r maxinterval$steps` steps.



## Imputing missing values
Unfortunately ouer data has missing values. 
...
```
To be specyfic we have `r missing_values_sum` missing values.Lets replace them by mean of steps for corresponding interval.


```{r, echo = TRUE}
activityfull<-activity %>% mutate(steps=ifelse(is.na(steps),average_steps_interval$steps,steps))
```
Once again lets look for histogram of total numbers of steps taken per day. This time for data without missing values.
...
Median is now equal `r total_steps_day_median_f` and mean is equal `r total_steps_day_mean_f`.
...
We can see that on weekends the nuber of steps during all day is more constant. On weekday we can observe pick about 8:30 and the smaller one after 18:00.
